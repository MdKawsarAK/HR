...
<div class="container py-4">
    ...
    <div class="card bg-primary text-white mb-3 shadow-sm">
        <div class="row">
            <div class="col-12 d-flex justify-content-between align-item-center ">
                <h3 class=" card-title text-white d-flex align-items-center  m-0">Type List</h3>
                <a href="{{ route('types.create') }}" class="btn btn-light btn-sm shadow-sm" title="Create New Product">
                    <i class="fa fa-plus mr-1"></i> Create New Type
                </a>
            </div>
        </div>
    </div>
    ...
    <a href="{{ route('types.show', $item->id) }}" class="btn btn-sm btn-info">View</a>
    <a href="{{ route('types.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>
    <form action="{{ route('types.destroy', $item->id) }}" method="POST" style="display:inline;">
        @csrf
        @method('DELETE')
